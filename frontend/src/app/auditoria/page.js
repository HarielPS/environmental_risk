...
const REQUIRED_DOCUMENTS = [
  {
    id: "epd",
    title: "Plantillas de desarrollo EPD",
    description:
      "Declaraciones ambientales de producto para el análisis de ciclo de vida.",
    icon: <FileText className="w-6 h-6" />,
  },
  {
    id: "co2",
    title: "CO₂ – nivel general de contaminantes",
    description:
      "Reporte detallado de emisiones de carbono estimadas para la obra.",
    icon: <CloudUpload className="w-6 h-6" />,
  },
  {
    id: "finance",
    title: "Reporte financiero de materiales",
    description:
      "Desglose de costes y procedencia de materiales de construcción.",
...
const App = () => {
  ...
  const [files, setFiles] = useState({ epd: null, co2: null, finance: null });
  const [uploading, setUploading] = useState({
    epd: false,
    co2: false,
    finance: false,
  });
  const [progress, setProgress] = useState({ epd: 0, co2: 0, finance: 0 });
  const [errors, setErrors] = useState({ epd: null, co2: null, finance: null });
  ...
  const uploadFilesToBackend = async (data) => {
    // Aquí es donde harías tu fetch o llamada a axios
    // const formData = new FormData();
    // formData.append('epd', data.epd); ...
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        if (Math.random() > 0.05) {
          resolve({ status: "success", message: "Análisis completado" });
        } else {
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      setToast(
        "Hubo un problema al procesar los archivos. Inténtalo de nuevo.",
      );
    } finally {
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm md:text-base mt-1">
              Necesitamos estos 3 PDFs para analizar tu proyecto y generar
